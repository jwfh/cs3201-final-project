...
def replacement(current_pop, current_fitnesses, offspring, offspring_fitnesses):
    population = []
    fitnesses = []
    sorted_pop, sorted_fit = sort_population(np.copy(current_pop), np.copy(current_fitnesses))
    for i in range(sorted_pop.shape[0] - offspring.shape[0]):
        population.append(sorted_pop[sorted_pop.shape[0]-i-1])
        fitnesses.append(sorted_fit[sorted_pop.shape[0]-i-1])
    return np.concatenate([np.asarray(population), offspring]), np.concatenate([np.asarray(fitnesses), offspring_fitnesses])